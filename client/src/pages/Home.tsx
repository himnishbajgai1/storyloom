...
function makeCard(image: string, index: number, name = `Photo ${String(index + 1).padStart(2, "0")}`, aiStatus: Card["aiStatus"] = "idle"): Card {
  return { id: `${Date.now()}-${index}-${Math.random()}`, image, name, kicker: "Your story", headline: "Add your story goal", caption: "Your AI-generated copy will appear here.", placement: index % 3 === 1 ? "center" : index % 3 === 2 ? "top" : "bottom", style: index % 2 ? "sans" : "serif", textTreatment: "glass", textColor: "#ffffff", gradientStart: "#18231f", gradientEnd: "#8da28f", gradientAngle: 180, overlayOpacity: 70, textSize: "medium", textAlign: "left", radius: "round", panelPaddingX: 16, panelPaddingY: 16, glassOpacity: 22, blurStrength: 18, borderOpacity: 28, shadowStrength: 22, lineHeight: 1, letterSpacing: -0.04, panelWidth: 84, panelOffsetX: 0, panelOffsetY: 0, aiStatus };
}

function normalizeCard(card: Partial<Card>, index: number): Card {
  return { ...makeCard(card.image ?? "", index, card.name ?? `Photo ${String(index + 1).padStart(2, "0")}`, card.aiStatus ?? "ready"), ...card, textTreatment: card.textTreatment ?? "glass", textColor: card.textColor ?? "#ffffff", gradientStart: card.gradientStart ?? "#18231f", gradientEnd: card.gradientEnd ?? "#8da28f", gradientAngle: card.gradientAngle ?? 180, overlayOpacity: card.overlayOpacity ?? 70, textSize: card.textSize ?? "medium", textAlign: card.textAlign ?? "left", radius: card.radius ?? "round", panelPaddingX: card.panelPaddingX ?? 16, panelPaddingY: card.panelPaddingY ?? 16, glassOpacity: card.glassOpacity ?? 22, blurStrength: card.blurStrength ?? 18, borderOpacity: card.borderOpacity ?? 28, shadowStrength: card.shadowStrength ?? 22, lineHeight: card.lineHeight ?? 1, letterSpacing: card.letterSpacing ?? -0.04, panelWidth: card.panelWidth ?? 84, panelOffsetX: card.panelOffsetX ?? 0, panelOffsetY: card.panelOffsetY ?? 0 } as Card;
}
  ...
  return <button onClick={onClick} style={{ borderRadius: card.radius === "round" ? "1.5rem" : ".8rem" }} className={`group relative aspect-[9/16] w-full overflow-hidden text-left story-shadow ${active ? "ring-2 ring-[#8da28f] ring-offset-4 ring-offset-[#f5f4ef]" : ""}`}>
    ...
    <div className={`absolute ${card.placement === "top" ? "top-6" : card.placement === "center" ? "top-1/2 -translate-y-1/2" : "bottom-7"}`} style={{ left: `${8 + card.panelOffsetX}%`, width: `${card.panelWidth}%`, transform: `${card.placement === "center" ? "translateY(-50%)" : ""} translate(${card.panelOffsetX / 2}%, ${card.panelOffsetY}px)` }}>
      <div className={`${radius} ${card.textTreatment === "plain" ? "" : "border"}`} style={{ color: card.textColor, textAlign: card.textAlign, background: card.textTreatment === "plain" ? "transparent" : panelBackground, backdropFilter: card.textTreatment === "plain" ? undefined : `blur(${card.blurStrength}px) saturate(150%)`, WebkitBackdropFilter: card.textTreatment === "plain" ? undefined : `blur(${card.blurStrength}px) saturate(150%)`, borderColor: card.textTreatment === "plain" ? "transparent" : `rgba(255,255,255,${card.borderOpacity / 100})`, boxShadow: card.textTreatment === "plain" ? "none" : panelShadow, padding: `${card.panelPaddingY}px ${card.panelPaddingX}px` }}>
        ...
        <h3 className={`max-w-full ${size} ${card.style === "serif" ? "font-display" : "font-semibold"}`} style={{ lineHeight: card.lineHeight, letterSpacing: `${card.letterSpacing}em` }}>{card.headline}</h3>
        <p className="mt-3 max-w-full text-[.7rem] opacity-80" style={{ lineHeight: card.lineHeight + .3, letterSpacing: `${card.letterSpacing / 2}em` }}>{card.caption}</p>
      </div>
    </div>
...
export default function Home() {
  ...
  const exportCard = async (card: Card) => {
    ...
    const panelW = canvas.width * (visual.panelWidth / 100); const panelX = (canvas.width - panelW) / 2 + visual.offsetX * 8; const panelH = 380 + visual.paddingY * 2; const panelY = y - 120 + visual.offsetY * 4; const textX = visual.align === "center" ? canvas.width / 2 : panelX + visual.paddingX; const maxTextWidth = panelW - visual.paddingX * 2;
    ...
    ctx.fillStyle = visual.textColor; ctx.textAlign = visual.align as CanvasTextAlign; const headlineSize = 88 * visual.headlineScale;
    ctx.font = card.style === "serif" ? `600 ${headlineSize}px Georgia` : `600 ${headlineSize - 4}px Arial`; wrapText(ctx, card.headline, textX, y, maxTextWidth, (headlineSize + 10) * visual.lineHeight, visual.align as CanvasTextAlign, visual.letterSpacing * headlineSize);
    ctx.font = "400 34px Arial"; wrapText(ctx, card.caption, textX, y + 170, maxTextWidth, 48 * (visual.lineHeight + .3), visual.align as CanvasTextAlign, visual.letterSpacing * 17);

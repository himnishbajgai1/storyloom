...
export default function Home() {
  ...
  const exportCard = async (card: Card) => {
    ...
    const headlineSizeForLayout = exportConfig.headlineScale; const captionSizeForLayout = exportConfig.captionScale; const panelH = exportConfig.panel.height; const panel = exportConfig.panel; const panelW = panel.width; const panelX = panel.x; const panelY = panel.y; const textX = visual.align === "center" ? canvas.width / 2 : panelX + visual.paddingX; const maxTextWidth = panelW - visual.paddingX * 2; const textY = panelY + visual.paddingY + visual.textScale * 1.15;
    ...
    const captionSize = fitCaptionScale({ baseScale: 30, caption: card.caption }); ctx.font = `400 ${captionSize}px ${canvasFontFamily(visual.fontFamily)}`; wrapText(ctx, card.caption, textX, textY + headlineSize * 2.4, maxTextWidth, captionSize * (visual.lineHeight + .25), visual.align as CanvasTextAlign, visual.letterSpacing * 15); ctx.globalAlpha = 1; ctx.shadowBlur = 0;

...
export default function Home() {
  ...
  const exportCard = async (card: Card) => {
    ...
    if (badge) { ctx.fillStyle = exportConfig.badgeColor; drawRoundedRect(ctx, panelX + visual.paddingX, blockTop, Math.min(250, maxTextWidth), 34, 8); ctx.fill(); ctx.fillStyle = readableTextColor(exportConfig.badgeColor); ctx.font = `700 18px monospace`; ctx.fillText(badge, panelX + visual.paddingX + 14, blockTop + 23); } if (kicker) { ctx.fillStyle = exportConfig.badgeColor; ctx.globalAlpha = .78; ctx.font = `500 16px monospace`; ctx.fillText(kicker.toUpperCase(), textX, blockTop + (badge ? 68 : 18)); ctx.globalAlpha = 1; }
    ...
    const captionSize = exportConfig.captionScale; ctx.font = `400 ${captionSize}px ${canvasFontFamily(visual.fontFamily)}`; const flow = storyCardVerticalFlow({ panelY, panelHeight: panelH, paddingY: visual.paddingY, headlineLines: exportConfig.headlineLines, headlineSize, captionLines: exportConfig.captionLines, captionSize, lineHeight: visual.lineHeight, showAction: Boolean(cta) }); const captionY = textY + flow.captionY; wrapText(ctx, card.caption, textX, captionY, maxTextWidth, captionSize * (visual.lineHeight + .25), visual.align as CanvasTextAlign, visual.letterSpacing * 15); ctx.globalAlpha = 1; ctx.shadowBlur = 0; ctx.shadowOffsetY = 0; ctx.shadowColor = "transparent";
    const actionY = Math.max(panelY + panelH - visual.paddingY - (cta ? 42 : 0), textY + flow.captionY + flow.captionToActionGap + flow.minimumGap); if (card.steps) { ctx.fillStyle = exportConfig.roleColor; drawRoundedRect(ctx, panelX + visual.paddingX, actionY - (cta ? 58 : 34), maxTextWidth, 42, 8); ctx.fill(); ctx.fillStyle = readableTextColor(exportConfig.roleColor); ctx.font = `700 16px monospace`; ctx.fillText(card.steps, panelX + visual.paddingX + 12, actionY - (cta ? 31 : 7)); } if (cta) { ctx.fillStyle = exportConfig.ctaColor; drawRoundedRect(ctx, panelX + visual.paddingX, actionY - 42, Math.min(maxTextWidth, 260), 42, 8); ctx.fill(); ctx.fillStyle = readableTextColor(exportConfig.ctaColor); ctx.font = `700 18px monospace`; ctx.fillText(cta, panelX + visual.paddingX + 16, actionY - 15); }

...
import { canUseSequenceActions, copyableCardStyle, createStoryCards, estimateTextLines, exportFilename, exportMetadata, exportRenderPlan, exportStyleConfig, fitCaptionScale, fitTextScale, moveCard, safePanelLayout, updateCard } from "@/lib/storySequence";
...
function StoryCard({ card, active, onClick }: { card: Card; active: boolean; onClick: () => void }) {
  ...
  return <button onClick={onClick} style={{ borderRadius: card.radius === "round" ? "1.5rem" : ".8rem" }} className={`group relative aspect-[9/16] w-full overflow-hidden text-left story-shadow ${active ? "ring-2 ring-[#8da28f] ring-offset-4 ring-offset-[#f5f4ef]" : ""}`}>
    ...
    <div className={`absolute ${card.placement === "top" ? "top-6" : card.placement === "center" ? "top-1/2 -translate-y-1/2" : "bottom-7"}`} style={{ left: `${8 + card.panelOffsetX}%`, width: `${card.panelWidth}%`, transform: `${card.placement === "center" ? "translateY(-50%)" : ""} translate(${card.panelOffsetX / 2}%, ${card.panelOffsetY}px)` }}>
      <div className={`${radius} ${card.textTreatment === "plain" ? "" : "border"}`} style={{ color: card.textColor, textAlign: card.textAlign, background: card.textTreatment === "plain" ? "transparent" : panelBackground, backdropFilter: card.textTreatment === "plain" ? undefined : `blur(${card.blurStrength}px) saturate(150%)`, WebkitBackdropFilter: card.textTreatment === "plain" ? undefined : `blur(${card.blurStrength}px) saturate(150%)`, borderColor: card.textTreatment === "plain" ? "transparent" : `rgba(255,255,255,${card.borderOpacity / 100})`, boxShadow: card.textTreatment === "plain" ? "none" : panelShadow, padding: `${card.panelPaddingY}px ${card.panelPaddingX}px` }}>
        <p className="eyebrow mb-2 opacity-75">{card.kicker}</p>
        <h3 className={`max-w-full ${card.style === "serif" ? "font-display" : "font-semibold"}`} style={{ fontSize: `${size}px`, lineHeight: card.lineHeight, letterSpacing: `${card.letterSpacing}em` }}>{card.headline}</h3>
        <p className="mt-3 max-w-full opacity-80" style={{ fontSize: `${fitCaptionScale({ baseScale: 11, caption: card.caption })}px`, lineHeight: card.lineHeight + .3, letterSpacing: `${card.letterSpacing / 2}em` }}>{card.caption}</p>
      </div>
    </div>
...
export default function Home() {
    ...
    try {
      const generated = await createStoryCards(cards, trimmedGoal, async (card, storyGoal) => {
        ...
        try {
          ...
          const result = await generateCopy.mutateAsync({ imageDataUrl: card.image, tone: "warm, thoughtful, quietly confident", goal: storyGoal, cardNumber: targetIndex + 1 });
  ...
  const generate = async (target: Card | undefined = active, storyGoal: string = goal.trim()) => {
    ...
    try {
      ...
      const result = await generateCopy.mutateAsync({ imageDataUrl: target.image, tone: "warm, thoughtful, quietly confident", goal: storyGoal, cardNumber: targetIndex + 1 });
  ...
  const exportCard = async (card: Card) => {
    ...
    const visual = exportStyleConfig(card); const metadata = exportMetadata(); const renderPlan = exportRenderPlan();
    ...
    const headlineSizeForLayout = fitTextScale({ baseScale: visual.textScale, headline: card.headline, caption: card.caption }); const captionSizeForLayout = fitCaptionScale({ baseScale: 30, caption: card.caption }); const headlineLines = estimateTextLines(card.headline, Math.max(10, Math.floor(maxPanelCharacters(visual.panelWidth, visual.paddingX, headlineSizeForLayout)))); const captionLines = estimateTextLines(card.caption, Math.max(18, Math.floor(maxPanelCharacters(visual.panelWidth, visual.paddingX, captionSizeForLayout)))); const panelH = 150 + visual.paddingY * 2 + headlineLines * headlineSizeForLayout * visual.lineHeight + captionLines * captionSizeForLayout * (visual.lineHeight + .25) + 80; const panel = safePanelLayout({ canvasWidth: canvas.width, canvasHeight: canvas.height, panelWidth: visual.panelWidth, panelHeight: panelH, offsetX: visual.offsetX, offsetY: visual.offsetY, placement: card.placement }); const panelW = panel.width; const panelX = panel.x; const panelY = panel.y; const textX = visual.align === "center" ? canvas.width / 2 : panelX + visual.paddingX; const maxTextWidth = panelW - visual.paddingX * 2; const textY = panelY + visual.paddingY + visual.textScale * 1.15;
    if (visual.treatment !== "plain") { ctx.save(); ctx.shadowColor = `rgba(0,0,0,${visual.shadowStrength / 160})`; ctx.shadowBlur = visual.shadowStrength; ctx.shadowOffsetY = visual.shadowStrength / 2; ctx.filter = visual.treatment === "blur" ? `blur(${Math.min(8, visual.blurStrength / 4)}px)` : "none"; ctx.fillStyle = visual.treatment === "blur" ? `rgba(12,18,16,${visual.glassOpacity / 100})` : `rgba(255,255,255,${visual.glassOpacity / 100})`; drawRoundedRect(ctx, panelX, panelY, panelW, panelH, visual.radius === "round" ? 42 : 20); ctx.fill(); ctx.filter = "none"; ctx.shadowColor = "transparent"; ctx.strokeStyle = `rgba(255,255,255,${visual.borderOpacity / 100})`; ctx.stroke(); ctx.restore(); }
    ctx.fillStyle = visual.textColor; if (renderPlan.drawWatermark) { ctx.font = "500 28px monospace"; ctx.fillText("STORYLOOM", 86, 108); } ctx.textAlign = visual.align as CanvasTextAlign; const headlineSize = fitTextScale({ baseScale: visual.textScale, headline: card.headline, caption: card.caption });
    ctx.font = card.style === "serif" ? `600 ${headlineSize}px Georgia` : `600 ${headlineSize - 4}px Arial`; wrapText(ctx, card.headline, textX, textY, maxTextWidth, (headlineSize + 8) * visual.lineHeight, visual.align as CanvasTextAlign, visual.letterSpacing * headlineSize);
    const captionSize = fitCaptionScale({ baseScale: 30, caption: card.caption }); ctx.font = `400 ${captionSize}px Arial`; wrapText(ctx, card.caption, textX, textY + headlineSize * 2.4, maxTextWidth, captionSize * (visual.lineHeight + .25), visual.align as CanvasTextAlign, visual.letterSpacing * 15);
  ...
  return <div className="min-h-screen paper-grid">
    ...
    <main className="mx-auto max-w-[1440px] px-5 py-8 lg:px-10 lg:py-12">
      ...
      <div className="mb-8 grid gap-4 rounded-[1.5rem] border border-black/10 bg-[#fbfaf7] p-5 shadow-[0_18px_50px_rgba(42,36,30,.05)] lg:grid-cols-[1fr_auto] lg:items-end"><label className="block"><span className="eyebrow text-[#77736c]">What should this story help you do?</span><textarea value={goal} onChange={e => setGoal(e.target.value)} rows={2} placeholder="Example: book more calls for my coaching offer, get sales for a new product, or drive people to a launch." className="mt-3 w-full resize-none rounded-xl border border-black/10 bg-[#f5f4ef] p-3 text-sm leading-relaxed outline-none focus:border-[#8da28f]" /></label><button onClick={() => void createStories()} disabled={creatingStories || !cards.length || !goal.trim()} className="inline-flex h-12 items-center justify-center gap-2 rounded-xl bg-[#252321] px-5 text-sm font-semibold text-white hover:bg-[#403c38] disabled:cursor-not-allowed disabled:opacity-40"><WandSparkles size={16} />{creatingStories ? "Creating stories…" : "Create stories"}</button></div>

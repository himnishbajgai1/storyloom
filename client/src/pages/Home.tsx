...
import { canUseSequenceActions, clampPanelDrag, copyableCardStyle, createStoryCards, estimateTextLines, exportCardConfig, exportCardRenderPlan, exportFilename, exportMetadata, exportRenderPlan, exportStyleConfig, fitCaptionScale, fitTextScale, exportCropRect, moveCard, normalizeCardVisibility, roleForCard, safePanelLayout, sequenceRoleOrder, shouldDrawTextPanel, storyPanelAnchor, updateCard, visualPresetStyle } from "@/lib/storySequence";
...
export default function Home() {
  ...
  const exportCard = async (card: Card) => {
    ...
    const panelH = exportConfig.panel.height; const panel = exportConfig.panel; const panelW = panel.width; const panelX = panel.x; const panelY = panel.y; const textX = visual.align === "center" ? canvas.width / 2 : panelX + visual.paddingX; const maxTextWidth = panelW - visual.paddingX * 2; const blockTop = panelY + visual.paddingY; const badge = card.showBadge ? card.badge : ""; const cta = card.showCta ? card.cta : ""; const headlineSize = fitTextScale({ baseScale: visual.textScale, headline: card.headline, caption: card.caption }); const textY = blockTop + (badge ? 52 : 0) + headlineSize * 1.15;
    ...
    if (exportConfig.watermark && renderPlan.drawWatermark) { ctx.fillStyle = "rgba(255,255,255,.7)"; ctx.font = "500 28px monospace"; ctx.fillText("STORYLOOM", 86, 108); } ctx.fillStyle = visual.textColor; ctx.textAlign = visual.align as CanvasTextAlign;
    ...
    const captionSize = exportConfig.captionScale; ctx.font = `400 ${captionSize}px ${canvasFontFamily(visual.fontFamily)}`; const captionY = textY + exportConfig.headlineLines * headlineSize * visual.lineHeight + captionSize * 0.85; wrapText(ctx, card.caption, textX, captionY, maxTextWidth, captionSize * (visual.lineHeight + .25), visual.align as CanvasTextAlign, visual.letterSpacing * 15); ctx.globalAlpha = 1; ctx.shadowBlur = 0;

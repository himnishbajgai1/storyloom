...
import { canUseSequenceActions, copyableCardStyle, createStoryCards, estimateTextLines, exportCardConfig, exportCardRenderPlan, exportFilename, exportMetadata, exportRenderPlan, exportStyleConfig, fitCaptionScale, fitTextScale, moveCard, roleForCard, safePanelLayout, sequenceRoleOrder, shouldDrawTextPanel, storyPanelAnchor, updateCard } from "@/lib/storySequence";
...
function maxPanelCharacters(panelWidth: number, paddingX: number, fontSize: number) {
  return Math.max(10, ((1080 * (panelWidth / 100)) - paddingX * 2) / Math.max(1, fontSize * 0.55));
}
...
function StoryCard({ card, active, onClick }: { card: Card; active: boolean; onClick: () => void }) {
  ...
  const textGlassStyle = card.textEffect === "solid" ? {} : { backgroundImage: `linear-gradient(135deg, ${card.textColor}, rgba(255,255,255,.58), ${card.textColor})`, WebkitBackgroundClip: "text", backgroundClip: "text", color: "transparent", textShadow: card.textEffect === "blurText" ? `0 0 ${Math.max(6, card.blurStrength / 2)}px ${card.textColor}` : "0 1px 0 rgba(255,255,255,.55)" };
  return <button onClick={onClick} style={{ borderRadius: card.radius === "round" ? "1.5rem" : ".8rem" }} className={`group relative aspect-[9/16] w-full overflow-hidden text-left story-shadow ${active ? "ring-2 ring-[#8da28f] ring-offset-4 ring-offset-[#f5f4ef]" : ""}`}>
    ...
    <div className="absolute" style={{ left: `${8 + card.panelOffsetX}%`, width: `${card.panelWidth}%`, top: anchor.top === null ? undefined : `${anchor.top * 100}%`, bottom: anchor.bottom === null ? undefined : `${anchor.bottom * 100}%`, transform: `${card.placement === "center" ? "translateY(-50%)" : ""} translate(${card.panelOffsetX / 2}%, ${card.panelOffsetY}px)` }}>
      <div className={`${radius} ${card.textTreatment === "plain" || card.textEffect !== "solid" ? "" : "border"}`} style={{ color: card.textColor, textAlign: card.textAlign, background: !textOnly && card.textTreatment !== "plain" ? panelBackground : "transparent", backdropFilter: textOnly || card.textTreatment === "plain" ? undefined : `blur(${card.blurStrength}px) saturate(150%)`, WebkitBackdropFilter: textOnly || card.textTreatment === "plain" ? undefined : `blur(${card.blurStrength}px) saturate(150%)`, borderColor: textOnly || card.textTreatment === "plain" ? "transparent" : `rgba(255,255,255,${card.borderOpacity / 100})`, boxShadow: textOnly || card.textTreatment === "plain" ? "none" : panelShadow, padding: `${card.panelPaddingY}px ${card.panelPaddingX}px` }}>
        {card.badge && <span className="mb-3 inline-flex w-fit rounded-md border border-white/45 bg-[#d7b27b]/90 px-3 py-1 font-mono text-[.58rem] font-bold tracking-[.18em] text-[#2a2118]">{card.badge}</span>}
        <p className="eyebrow mb-2 opacity-75">{card.kicker}</p>
        <h3 className={`max-w-full ${fontClass(card.fontFamily)} ${card.style === "sans" ? "font-semibold" : ""}`} style={{ ...textGlassStyle, fontSize: `${size}px`, lineHeight: card.lineHeight, letterSpacing: `${card.letterSpacing}em` }}>{card.headline}</h3>
        <p className="mt-3 max-w-full opacity-80" style={{ ...textGlassStyle, fontSize: `${fitCaptionScale({ baseScale: 11, caption: card.caption })}px`, lineHeight: card.lineHeight + .3, letterSpacing: `${card.letterSpacing / 2}em` }}>{card.caption}</p>
        {card.steps && <div className="mt-4 flex items-center justify-between gap-2 rounded-lg border border-[#d7b27b]/80 bg-black/25 px-3 py-2 font-mono text-[.52rem] font-bold uppercase tracking-[.08em] text-white/95"><span>{card.steps}</span></div>}
        {card.cta && <div className="mt-5 inline-flex items-center rounded-md bg-[#d7b27b] px-4 py-2 font-mono text-[.62rem] font-bold uppercase tracking-[.1em] text-[#241d17]">{card.cta}</div>}
      </div>
    </div>
...
export default function Home() {
  ...
  const exportCard = async (card: Card) => {
    ...
    const visual = exportStyleConfig(card); const renderPlan = exportRenderPlan(); const exportConfig = exportCardConfig(card);
    const imageAspect = image.width / image.height; const canvasAspect = canvas.width / canvas.height; const baseSourceWidth = imageAspect > canvasAspect ? image.height * canvasAspect : image.width; const baseSourceHeight = imageAspect > canvasAspect ? image.height : image.width / canvasAspect; const sourceWidth = baseSourceWidth / card.imageZoom; const sourceHeight = baseSourceHeight / card.imageZoom; const sourceX = Math.max(0, Math.min(image.width - sourceWidth, image.width * (card.imagePositionX / 100) - sourceWidth / 2)); const sourceY = Math.max(0, Math.min(image.height - sourceHeight, image.height * (card.imagePositionY / 100) - sourceHeight / 2));
    ...
    const headlineSizeForLayout = exportConfig.headlineScale; const captionSizeForLayout = exportConfig.captionScale; const panelH = exportConfig.panel.height; const panel = exportConfig.panel; const panelW = panel.width; const panelX = panel.x; const panelY = panel.y; const textX = visual.align === "center" ? canvas.width / 2 : panelX + visual.paddingX; const maxTextWidth = panelW - visual.paddingX * 2; const blockTop = panelY + visual.paddingY; const textY = blockTop + (card.badge ? 52 : 0) + visual.textScale * 1.15;
    if (exportConfig.drawPanel) { ctx.save(); ctx.shadowColor = `rgba(0,0,0,${visual.shadowStrength / 160})`; ctx.shadowBlur = visual.shadowStrength; ctx.shadowOffsetY = visual.shadowStrength / 2; ctx.filter = visual.treatment === "blur" ? `blur(${Math.min(8, visual.blurStrength / 4)}px)` : "none"; ctx.fillStyle = visual.treatment === "blur" ? `rgba(12,18,16,${visual.glassOpacity / 100})` : `rgba(255,255,255,${visual.glassOpacity / 100})`; drawRoundedRect(ctx, panelX, panelY, panelW, panelH, visual.radius === "round" ? 42 : 20); ctx.fill(); ctx.filter = "none"; ctx.shadowColor = "transparent"; ctx.strokeStyle = `rgba(255,255,255,${visual.borderOpacity / 100})`; ctx.stroke(); ctx.restore(); }
    if (card.badge) { ctx.fillStyle = "rgba(215,178,123,.95)"; drawRoundedRect(ctx, panelX + visual.paddingX, blockTop, Math.min(250, maxTextWidth), 34, 8); ctx.fill(); ctx.fillStyle = "#2a2118"; ctx.font = `700 18px monospace`; ctx.fillText(card.badge, panelX + visual.paddingX + 14, blockTop + 23); }
    if (exportConfig.watermark && renderPlan.drawWatermark) { ctx.fillStyle = "rgba(255,255,255,.7)"; ctx.font = "500 28px monospace"; ctx.fillText("STORYLOOM", 86, 108); } ctx.fillStyle = visual.textColor; ctx.textAlign = visual.align as CanvasTextAlign; const headlineSize = fitTextScale({ baseScale: visual.textScale, headline: card.headline, caption: card.caption });
    if (visual.textEffect !== "solid") { ctx.globalAlpha = visual.textEffect === "blurText" ? .78 : .9; ctx.shadowColor = visual.textColor; ctx.shadowBlur = visual.textEffect === "blurText" ? Math.max(8, visual.blurStrength / 2) : 2; }
    ...
    const captionSize = exportConfig.captionScale; ctx.font = `400 ${captionSize}px ${canvasFontFamily(visual.fontFamily)}`; wrapText(ctx, card.caption, textX, textY + headlineSize * 2.4, maxTextWidth, captionSize * (visual.lineHeight + .25), visual.align as CanvasTextAlign, visual.letterSpacing * 15); ctx.globalAlpha = 1; ctx.shadowBlur = 0;
    const actionY = panelY + panelH - visual.paddingY - (card.cta ? 42 : 0); if (card.steps) { ctx.fillStyle = "rgba(0,0,0,.3)"; drawRoundedRect(ctx, panelX + visual.paddingX, actionY - (card.cta ? 58 : 34), maxTextWidth, 42, 8); ctx.fill(); ctx.fillStyle = "#ffffff"; ctx.font = `700 16px monospace`; ctx.fillText(card.steps, panelX + visual.paddingX + 12, actionY - (card.cta ? 31 : 7)); } if (card.cta) { ctx.fillStyle = "#d7b27b"; drawRoundedRect(ctx, panelX + visual.paddingX, panelY + panelH - visual.paddingY - 42, Math.min(maxTextWidth, 260), 42, 8); ctx.fill(); ctx.fillStyle = "#241d17"; ctx.font = `700 18px monospace`; ctx.fillText(card.cta, panelX + visual.paddingX + 16, panelY + panelH - visual.paddingY - 15); }
    ...
    <main className="mx-auto max-w-[1440px] px-5 py-8 lg:px-10 lg:py-12">
      ...
      <div className="grid gap-10 xl:grid-cols-[minmax(0,1fr)_360px]">
        ...
        <aside className="sticky top-24 self-start"><div className="mb-4 rounded-[1.5rem] border border-black/10 bg-[#252321] p-3 shadow-[0_18px_50px_rgba(42,36,30,.12)]"><p className="eyebrow mb-2 px-1 text-white/55">Live preview</p>{active ? <StoryCard card={active} active={false} onClick={() => undefined} /> : <div className="grid aspect-[9/16] place-items-center rounded-2xl bg-white/10 text-sm text-white/60">Upload a photo to preview</div>}</div><div className="rounded-[1.5rem] border border-black/10 bg-[#fbfaf7] p-5 shadow-[0_18px_50px_rgba(42,36,30,.07)]"><div className="mb-6 flex items-center justify-between"><div><p className="eyebrow text-[#77736c]">Edit card</p><p className="mt-2 font-display text-2xl">{active ? `Card ${String(activeIndex + 1).padStart(2, "0")}` : "No card selected"}</p></div><button className="rounded-full p-2 text-[#77736c] hover:bg-[#e8e5dc]"><MoreHorizontal size={18} /></button></div>{active && <div className="mb-5 grid grid-cols-2 gap-2"><button onClick={copyStyle} className="rounded-xl border border-black/10 bg-[#f5f4ef] px-3 py-2 text-xs font-semibold hover:bg-[#eaf1e8]">Copy style</button><button onClick={pasteStyle} disabled={!styleClipboard} className="rounded-xl border border-black/10 bg-[#f5f4ef] px-3 py-2 text-xs font-semibold hover:bg-[#eaf1e8] disabled:cursor-not-allowed disabled:opacity-40">Paste style</button></div>}{active ? <div className="space-y-5">
            ...
            <div className="grid gap-3 sm:grid-cols-2"><label className="block"><span className="eyebrow text-[#77736c]">Badge / eyebrow</span><input value={active.badge} onChange={e => updateActive({ badge: e.target.value })} className="mt-2 w-full rounded-xl border border-black/10 bg-[#f5f4ef] px-3 py-2 text-xs outline-none focus:border-[#8da28f]" /></label><label className="block"><span className="eyebrow text-[#77736c]">CTA button</span><input value={active.cta} onChange={e => updateActive({ cta: e.target.value })} placeholder="Watch now →" className="mt-2 w-full rounded-xl border border-black/10 bg-[#f5f4ef] px-3 py-2 text-xs outline-none focus:border-[#8da28f]" /></label></div><label className="block"><span className="eyebrow text-[#77736c]">Kicker</span><input value={active.kicker} onChange={e => updateActive({ kicker: e.target.value })} className="mt-2 w-full border-b border-black/15 bg-transparent py-2 text-sm outline-none focus:border-[#8da28f]" /></label>

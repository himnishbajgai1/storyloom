...
describe("story persistence access", () => {
  ...
  it("saves with the signed-in user id and returns the created id", async () => {
    ...
    const result = await caller.story.save({ name: "My story", cards: [{ id: "1", headline: "Hello" }] });
    expect(result).toEqual({ id: 42, name: "My story" });
    expect(createStorySequence).toHaveBeenCalledWith(expect.objectContaining({ userId: 7, name: "My story" }));
  });

  it("lists and parses only the signed-in user’s stored cards", async () => {
    getStorySequencesByUserId.mockResolvedValueOnce([{ id: 42, userId: 7, name: "My story", cardsJson: JSON.stringify([{ id: "1" }]), createdAt: new Date(), updatedAt: new Date() }]);
    const caller = appRouter.createCaller(authenticatedContext);
    const result = await caller.story.listMine();
    expect(getStorySequencesByUserId).toHaveBeenCalledWith(7);
    expect(result[0]?.cards).toEqual([{ id: "1" }]);
  });
});

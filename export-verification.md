...
## Element color export verification

The live preview and export renderer now both use `roleColor` for the visible step-ribbon block, while `badgeColor` controls the badge and kicker and `ctaColor` controls the CTA button. A fresh 1080×1920 export, `/home/ubuntu/Downloads/pasted-file-e5ge9r-image (4).png`, was generated after applying the distinct color values `#ff7a59`, `#123456`, and `#e7f0d0`. The artifact opened successfully; the selected role block uses the role color contract, text contrast is computed against each chosen color, and the export remains watermark-free. The generated card did not contain visible badge or CTA content in this particular run, so their exact colored pixels were validated through the export contract test rather than this artifact.
...
## Remaining download mismatch reproduction and final repair — 2026-08-13

The exact failure was reproduced from the live upload-and-goal flow. The downloaded PNG reused small preview CSS pixels on the 1080×1920 canvas, and its caption baseline double-counted the headline offset. The export renderer now scales typography, padding, badge, kicker, ribbon, and CTA geometry into the 1080 canvas, adapts scale to the safe frame, and derives caption position once from the shared flow metrics.

The latest same-run pair was `/home/ubuntu/Downloads/live-preview-e5ge9r.png` and `/home/ubuntu/Downloads/img-7042 (8).png`. The PNG is 1080×1920. Inspection confirms the headline is dominant and readable, the badge and kicker sit above it, the caption begins below the headline without overlap, and the CTA remains fully visible below the caption/action flow. The crop and panel anchor remain stable. The uploaded `IMG_7042.webp` contains faint pre-existing lettering in the source photo; that lettering is not generated by Storyloom and remains part of the source image.
